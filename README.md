...
AES-256-GCM encryption — all with an in-app activity log so you can see exactly
what the app is doing.

## Features

- **File browser** — navigate, search, multi-select, copy/move (cut+paste),
  rename, delete, create folders, sort by name/date/size.
- **Tap to open, anywhere** — tapping a regular file opens it in whatever app
  your phone already uses for it (photos, PDFs, docs, etc.), just like a
  normal file manager.
- **Per-item menu** — long-press-free access via a ⋮ button on every row:
  open, "open with…", share, lock with a password, details, rename, copy,
  cut, delete.
- **Share** — send one or more files straight to another app (Messages,
  email, etc.) from the per-item menu or the multi-select toolbar.
- **File details** — type, size, modified date, and location at a glance.
- **Quick access** — one-tap shortcuts to Downloads, Pictures, DCIM,
  Documents, Movies and Music from the home screen (only shown for folders
  that actually exist on your device).
...
- **Storage permission flow** — asks for "All files access" on Android 11+
  ...
  clear bullet-point explanation screen and a re-check whenever you return to
  the app.
- **In-app activity log** — every operation (permissions, browsing, compress/
...
- **"All files access" toggle doesn't stick on some devices** — this is a
  ...
  in-app activity log (accessible from the permission screen and the main menu)
