        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('ArchiveX needs this to:', style: TextStyle(fontWeight: FontWeight.w600)),
              const SizedBox(height: 10),
              const Align(
                alignment: Alignment.centerLeft,
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  _Bullet('Browse and open your files and folders'),
                  _Bullet('Create and extract zip / tar archives'),
                  _Bullet('Lock files with a password (real AES-256 encryption)'),
                ]),
              ),
...
class _BrowserPageState extends State<BrowserPage> {
  ...
  void _load() {
    try {
      ...
      l.sort((a, b) {
        ...
        switch (sortBy) {
          case SortBy.date:
            cmp = (s[a.path]?.modified ?? DateTime(0)).compareTo(s[b.path]?.modified ?? DateTime(0));
            break;
          case SortBy.size:
            cmp = (s[a.path]?.size ?? 0).compareTo(s[b.path]?.size ?? 0);
            break;
          case SortBy.name:
  ...
  Future<void> _openEntity(FileSystemEntity e) async {
    ...
    if (isArchive(e.path)) {
      ...
      await _openWith(e.path);
    }
  }

  Future<void> _openWith(String path) async {
    AppLog.i('open', 'open with: $path');
    final result = await OpenFilex.open(path);
    if (result.type != ResultType.done) {
      final why = result.message.isNotEmpty ? result.message : 'no app found for this file type';
      _toast('Could not open ${p.basename(path)} ($why)');
      AppLog.e('open', '$path -> ${result.type}: ${result.message}');
    }
  }

  Future<void> _share(List<String> paths) async {
    if (paths.isEmpty) return;
    AppLog.i('share', 'sharing ${paths.length} item(s)');
    try {
      await SharePlus.instance.share(ShareParams(files: paths.map((e) => XFile(e)).toList()));
    } catch (e, st) {
      AppLog.e('share', '$e', st);
  ...
  Future<void> _shareSelected() async {
    final files = sel.where(FileSystemEntity.isFileSync).toList();
    if (files.isEmpty) {
      _toast("Select at least one file to share — folders can't be shared directly");
      return;
    }
    await _share(files);
  }
  ...
  Widget _detailRow(String k, String v) => Padding(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          SizedBox(width: 76, child: Text(k, style: const TextStyle(color: Colors.grey, fontSize: 13))),
          Expanded(child: Text(v, style: const TextStyle(fontSize: 13))),
        ]),
      );

  Future<void> _showDetails(FileSystemEntity e) async {
    ...
    if (isDir) {
      try {
        itemCount = Directory(e.path).listSync().length;
      } catch (_) {}
    }
    if (!mounted) return;
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          _detailRow('Type', isDir
              ? 'Folder'
              : kindOf(e.path) == Kind.aes
                  ? 'Encrypted file'
                  : isArchive(e.path)
                      ? 'Archive'
                      : 'File'),
          if (!isDir && st != null) _detailRow('Size', fmtSize(st.size)),
          if (isDir && itemCount >= 0) _detailRow('Items', '$itemCount'),
          if (st != null) _detailRow('Modified', fmtDate(st.modified)),
          _detailRow('Location', p.dirname(e.path)),
        ]),
  ...
  Future<void> _showItemMenu(FileSystemEntity e) async {
    final isDir = e is Directory;
    final k = isDir ? null : kindOf(e.path);
      ...
      builder: (_) => SafeArea(
        child: Wrap(children: [
          ListTile(
            ...
            title: Text(p.basename(e.path), overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.bold)),
          ),
          const Divider(height: 1),
          if (isDir)
            ListTile(leading: const Icon(Icons.folder_open), title: const Text('Open'), onTap: () => Navigator.pop(context, 'open')),
          if (!isDir && (isArchive(e.path) || k == Kind.aes))
            ListTile(
              leading: Icon(k == Kind.aes ? Icons.lock_open : Icons.unarchive),
              title: Text(k == Kind.aes ? 'Decrypt' : 'View / extract'),
              onTap: () => Navigator.pop(context, 'open'),
            ),
          if (!isDir && k == Kind.other)
            ListTile(leading: const Icon(Icons.open_in_new), title: const Text('Open with…'), onTap: () => Navigator.pop(context, 'openwith')),
          if (!isDir) ListTile(leading: const Icon(Icons.share_outlined), title: const Text('Share'), onTap: () => Navigator.pop(context, 'share')),
          if (!isDir && k != Kind.aes)
            ListTile(leading: const Icon(Icons.enhanced_encryption_outlined), title: const Text('Lock with a password'), onTap: () => Navigator.pop(context, 'encrypt')),
          ListTile(leading: const Icon(Icons.info_outline), title: const Text('Details'), onTap: () => Navigator.pop(context, 'details')),
          ListTile(leading: const Icon(Icons.drive_file_rename_outline), title: const Text('Rename'), onTap: () => Navigator.pop(context, 'rename')),
          ListTile(leading: const Icon(Icons.copy_outlined), title: const Text('Copy'), onTap: () => Navigator.pop(context, 'copy')),
          ListTile(leading: const Icon(Icons.content_cut), title: const Text('Cut'), onTap: () => Navigator.pop(context, 'cut')),
          ListTile(
    ...
    switch (action) {
      ...
      case 'copy':
        ...
        _toast('Copied — open the destination folder and tap paste');
        break;
      case 'cut':
        setState(() {
          clip = [e.path];
        ...
        _toast('Cut — open the destination folder and tap paste');
        break;
      case 'delete':
        sel
          ..clear()
  ...
  Widget _quickAccessRow() {
    const candidates = {
      'DCIM': Icons.camera_alt_outlined,
    ...
    final folders = candidates.entries.where((c) => Directory(p.join(root, c.key)).existsSync()).toList();
    ...
    return SizedBox(
      ...
      child: ListView(
        ...
        children: [
          for (final c in folders)
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 4),
              child: ActionChip(
                avatar: Icon(c.value, size: 18),
                label: Text(c.key),
                onPressed: () => _go(p.join(root, c.key)),
              ),
            ),
        ],
      ),
  ...
  Widget _emptyState() => Center(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(query.isNotEmpty ? Icons.search_off : Icons.folder_off_outlined, size: 64, color: Colors.grey),
            const SizedBox(height: 12),
            Text(
              query.isNotEmpty ? 'No files match "$query"' : 'Nothing here yet',
            ...
            if (query.isEmpty)
              const Padding(
                ...
                child: Text(
                  'Use the menu above to make a new folder,\nor paste in files you copied.',
                  textAlign: TextAlign.center,
                  style: TextStyle(color: Colors.grey),
                ),
              ),
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: selecting
              ? [
                  ...
                  IconButton(icon: const Icon(Icons.share_outlined), tooltip: 'Share', onPressed: _shareSelected),
                  IconButton(
              ...
              : [
                  ...
                  PopupMenuButton<String>(
                    onSelected: (v) {
                      ...
                      const map = {'sort_name': SortBy.name, 'sort_date': SortBy.date, 'sort_size': SortBy.size};
                    ...
                    itemBuilder: (_) => [
                      ...
                      CheckedPopupMenuItem(value: 'sort_name', checked: sortBy == SortBy.name, child: const Text('Sort by name')),
                      CheckedPopupMenuItem(value: 'sort_date', checked: sortBy == SortBy.date, child: const Text('Sort by date')),
                      CheckedPopupMenuItem(value: 'sort_size', checked: sortBy == SortBy.size, child: const Text('Sort by size')),
                      const PopupMenuDivider(),
                      const PopupMenuItem(value: 'log', child: Text('Activity log')),
                    ],
                  ),
                ],
        ),
        body: Column(
          children: [
            if (!selecting && !searching && cwd.path == root) _quickAccessRow(),
            Expanded(
              child: shown.isEmpty
                  ? _emptyState()
                  : ListView.builder(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return ListTile(
                          ...
                          subtitle: st == null
                              ? null
                              : Text('${e is Directory ? '' : '${fmtSize(st.size)} · '}${fmtDate(st.modified)}'),
                          trailing: selecting
                              ? null
                              : IconButton(
                                  icon: const Icon(Icons.more_vert),
                                  tooltip: 'More',
                                  onPressed: () => _showItemMenu(e),
                                ),
                          onTap: () => _openEntity(e),
